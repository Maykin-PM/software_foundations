...
Compute normalize (B1 (B0 Z)).
(* FILL IN HERE *)
Example normal_test : normalize (B1 (B0 (B0 Z))) = B1 Z.
Proof. simpl. reflexivity. Qed.
...
Lemma nat_to_bin2n: forall n, nat_to_bin ( n + n ) = double_bin (nat_to_bin (n)). 
Proof.
  intros n.
  induction n as [| n' IHn'].
  - simpl. reflexivity.
  - simpl. rewrite double_incr_bin. rewrite add_comm. simpl. rewrite IHn'. reflexivity.
Qed. 

Lemma nat_to_bin2np1: forall n, nat_to_bin ( n + n + 1) = incr (double_bin (nat_to_bin (n))). 
Proof.
  intros n.
  induction n.
  - simpl. reflexivity.
  -  rewrite add_comm.  simpl. 
    ...
    rewrite H. rewrite add_assoc. rewrite IHn.
...
Proof.
  ...
  induction b. 
  - simpl. reflexivity.
  - simpl. rewrite <- IHb. rewrite plus_n_O.  rewrite nat_to_bin2n. reflexivity.
  - simpl. rewrite <- IHb.  assert (H: bin_to_nat b + 0 = bin_to_nat b). { rewrite add_comm. reflexivity. }
    rewrite H. rewrite nat_to_bin2np1. reflexivity.
Qed.
