...
Lemma map_lin : forall (X Y : Type) (f : X -> Y) (l1 l2: list X), map f(l1 ++ l2) = map f l1 ++ map f l2. 
Proof. 
  intros X Y f l1 l2.
...
Proof.
  ...
  induction l as [| x l' IHl'].
  - simpl. reflexivity.
  - simpl. rewrite map_lin. rewrite IHl'. simpl. reflexivity.
Qed.
...
(* Definition exp (n m : cnat) : cnat
  := 
  fun (X:Type) (f:X->X) (x:X) => m (X->X) (fun f => (fun f x => f (f x)) f) (n X f) x. *)
